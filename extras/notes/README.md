...
|------|----------|---------|--------|-------|
| [sip_daily_observations.md](sip_daily_observations.md) | SIP | Running log of notable B2BUA/SIP observations (newest first). 2026-06-17: durable trace log (`data/b2bua_trace.log`) + embassy call-tree bait wired up; monetization now active on FR/AR/ES (not Palestine); ab00day operator pumps from a sibling IP (AS47154); RTP-pool exhaustion from one York flooder; Saudi-embassy `ack_then_bye` from a Frontier residential IP (candidate audio-validator). | Ongoing | 2026-06-17 → |
